...
import { getFirestore, collection, doc, getDocs, getDoc, setDoc, addDoc, updateDoc, deleteDoc, onSnapshot, query, orderBy, limit, serverTimestamp } from "https://www.gstatic.com/firebasejs/12.6.0/firebase-firestore.js";
        ...
        window.router = (pageId) => {
            ...
            try { const u = new URL(window.location); u.searchParams.set('page', pageId); window.history.pushState({ page: pageId }, '', u); } catch(e){}
        ...
        function loadPageFromUrl() { try{ router(new URLSearchParams(window.location.search).get('page')||"FrontPage"); }catch(e){router("FrontPage");} }
        window.onpopstate = (e) => { if(e.state?.page) fetchDocument(e.state.page); };
